Feature: only about page related to cluster login page
  ...
  Scenario: Check vpcCIDR , podCIDR and serviceCIDR textbox on UI page.
    ...
    When I perform the :switch_to_osd_creation_page web action with:
      |parameter_needed|no|
    When I perform the :select_advanced_radio_button web action with:
      |parameter_needed|no|
    Then the step should succeed
    ...
    Then the step should succeed
    When I perform the :check_host_prefix_error_messages web action with:
      | param_needed | no |
    Then the step should succeed
    ...
    #  Then the step should succeed
    ########################################################
    When I perform the :select_basic_radio_button web action with:
      |parameter_needed|no|
    Then the step should succeed
    When I perform the :click_create_button web action with:
      |parameter_needed|no|
    Then the step should succeed
    When I perform the :cluster_detail_page_loaded web action with:
      |parameter_needed|no|
    Then the step should succeed
  ...
  Scenario: Checking hover message for disabled machine types when creating OSD cluster
    ...
    Then the step should succeed
    When I perform the :select_multi_az web action with:
      |parameter_needed|no|
    Then the step should succeed
    When I perform the :check_no_quota_tooltip web action with:
      |machine_type|c5.2xlarge|
  ...
  Scenario: There will be validation for AWS credentials for BYOC cluster creation
    ...
    When I perform the :select_byoc_model web action with:
      |parameter_needed|no|
    Then the step should succeed
    When I perform the :check_no_input_errors_to_required_aws_items web action with:
      |parameter_needed|no|
    Then the step should succeed
