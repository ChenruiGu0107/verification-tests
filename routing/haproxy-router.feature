Feature: Testing haproxy router
  ...
  Scenario: The routekey should use underline instead of dash as connector
    ...
    When I run the :create client command with:
      |f|<%= BushSlicer::HOME %>/features/tierN/testdata/routing/tc/tc505814/route_unsecure.json|
    Then the step should succeed
    When I run the :create client command with:
      |f|<%= BushSlicer::HOME %>/features/tierN/testdata/routing/tc/tc505814/route_edge.json|
    Then the step should succeed
    When I run the :create client command with:
      |f|<%= BushSlicer::HOME %>/features/tierN/testdata/routing/tc/tc505814/route_reencrypt.json|
    Then the step should succeed
    When I run the :create client command with:
      |f|<%= BushSlicer::HOME %>/features/tierN/testdata/routing/tc/tc505814/route_pass.json|
    Then the step should succeed
  ...
  Scenario: haproxy logs can be sent to syslog for hostnetwork mode
    ...
    And I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/routing/passthrough/route_pass.json |
    Then the step should succeed
  ...
  Scenario: can set cookie name for reencrypt routes by annotation
    ...
    Given I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/routing/reencrypt/two-caddy-with-serving-cert.yaml |
    Then the step should succeed
    And all pods in the project are ready
    When I run the :create_route_reencrypt client command with:
      | name    | reen-route     |
