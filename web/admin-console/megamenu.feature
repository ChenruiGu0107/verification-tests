Feature: mega menu on console
  ...
  Scenario: Allow users to Report Bug and Open Support Case from console
    ...
    And evaluation of `cluster_version('version').version` is stored in the :cluster_detail_version clipboard
    And evaluation of `cluster_version('version').cluster_id` is stored in the :cluster_id clipboard

    Given the first user is cluster-admin
    And I open admin console in a browser
    When I perform the :check_report_bug_or_open_case_link web action with:
      | bz_version     | <%= cb.master_version %>         |
      | detail_version | <%= cb.cluster_detail_version %> |
