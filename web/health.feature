Feature: Health related feature on web console
  ...
  Scenario Outline: Check, set and remove readiness and liveness probe for dc and standalone rc in web
    ...
    When I run the :create client command with:
      | f    |  <%= BushSlicer::HOME %>/features/tierN/testdata/deployment/<kind>-with-two-containers.yaml |
  ...
  Scenario: Health Check for k8s deployment
    Given I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/deployment/tc536593/deployment-with-two-containers.yaml |
    Then the step should succeed
  ...
  Scenario: Health Check for k8s replicaset
    Given I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/replicaSet/tc536594/replicaset-with-two-containers.yaml |
    Then the step should succeed
    Given 1 pods become ready with labels:
      | app=guestbook |
