Feature: Testing registry
  ...
  Scenario: Add/Remove role to/from project on atomic-registry console
    ...
    When I run the :goto_projects_page web action
    Then the step should succeed
    And I perform the :select_project_dropdown_in_iframe web action with:
      | project_name | <%= project.name %> |
    Then the step should succeed
    Given evaluation of `user(1, switch: false).name` is stored in the :second_user clipboard
    When I perform the :add_member_to_project_in_iframe web action with:
      | member_name | <%= cb.second_user %> |
      | role_type   | Admin                 |
    Then the step should succeed
    Given I wait up to 60 seconds for the steps to pass:
    """
    Given the expression should be true> role_binding("registry-admin").user_names(cached: false).include? "<%= cb.second_user %>"
    """
    ...
    Given I wait up to 60 seconds for the steps to pass:
    """
    Given the expression should be true> role_binding("registry-editor").user_names(cached: false).include? "<%= cb.second_user %>"
    """
    ...
    Given I wait up to 60 seconds for the steps to pass:
    """
    Given the expression should be true> role_binding("registry-viewer").user_names(cached: false).include? "<%= cb.second_user %>"
    """

    When I perform the :remove_member_from_project_in_iframe web action with:
      | member_name | <%= cb.second_user %> |
    Then the step should succeed
    ...
    When I run the :get client command with:
      ...
      | n        | <%= project.name %> |
    Then the step should succeed
    And the output should not contain "<%= cb.second_user %>"
    """
