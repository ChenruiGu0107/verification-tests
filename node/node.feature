Feature: Node management
  ...
  Scenario: Specify unsafe namespaced kernel parameters for pod with invalid value
    ...
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/pods/sysctls/pod-sysctl-unsafe-invalid1.yaml |
    Then the step should fail
    And the output should match:
      | Invalid value: "invalid": sysctl "invalid" not of the format sysctl_name |
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/pods/sysctls/pod-sysctl-unsafe-invalid3.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/pods/sysctls/pod-sysctl-unsafe-invalid2.yaml |
    Then the step should succeed
    And I wait for the steps to pass:
    """
    When I run the :describe client command with:
  ...
  Scenario: Specify unsafe namespaced kernel parameters for pod
    Given I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/pods/sysctls/pod-sysctl-unsafe.yaml |
    Then the step should succeed
    And I wait for the steps to pass:
    """
    When I run the :get client command with:
  ...
  Scenario: Kubelet should remove disk backed emptydir volumes when pod is terminated
    Given I have a project
    When I run the :create client command with:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/pods/emtydir-host.yaml |
    Then the step should succeed
