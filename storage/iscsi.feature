Feature: ISCSI volume plugin testing
  ...
  Scenario: Multiple iSCSI LUNs with rw and ro mode should ensure the access behavior correctly
    ...
    Given admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pv-read-write.json" where:
    ...
    And I create a manual pvc from "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pvc-read-write.json" replacing paths:
    ...
    Given admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pv-read-only.json" where:
      | ["metadata"]["name"]                      | iscsi-ro-<%= project.name %>  |
      | ["spec"]["iscsi"]["targetPortal"]         | <%= cb.iscsi_ip %>:3260       |
      | ["spec"]["iscsi"]["initiatorName"]        | iqn.2016-04.test.com:test.img |
      | ["spec"]["persistentVolumeReclaimPolicy"] | Retain                        |
    And I create a manual pvc from "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pvc-read-only.json" replacing paths:
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pod-two-luns.json" replacing paths:
  ...
  Scenario: A pod with multiple containers reference the same iscsi volume (ROX and ROW)
    ...
    When I run oc create over "https://raw.githubusercontent.com/openshift-qe/docker-iscsi/master/pod-with-multicontainer.yaml" replacing paths:
      ...
      | ["spec"]["volumes"][1]["iscsi"]["initiatorName"] | iqn.2016-04.test.com:test.img                          |
  ...
  Scenario: Namespaced iscsi chap secrets
    ...
    When I run the :create client command with:
      | filename  | <%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/chap-secret-auto.yml |
    Then the step should succeed

    Given I create a new project
    When admin creates a PV from "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/iscsi/pv-chap.json" where:
