Feature: CephFS storage plugin testing
  ...
  Scenario: Namespaced CephFS secrets
    ...
    Given admin creates a PV from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/cephfs/pv-retain.json" where:
      | ["metadata"]["name"]                         | pv-cephfs-server-<%= project.name %>                |
    ...
    And I create a manual pvc from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/cephfs/pvc-cephfs.json" replacing paths:
      | ["metadata"]["name"]   | pvc-cephfs-<%= project.name %>       |
      | ["spec"]["volumeName"] | pv-cephfs-server-<%= project.name %> |
    Then the step should succeed
    And the "pvc-cephfs-<%= project.name %>" PVC becomes bound to the "pv-cephfs-server-<%= project.name %>" PV
    ...
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/cephfs/pod.json" replacing paths:
      | ["metadata"]["name"]                                         | cephfs-<%= project.name %>     |
      | ["spec"]["volumes"][0]["persistentVolumeClaim"]["claimName"] | pvc-cephfs-<%= project.name %> |
    Then the step should succeed
    And the pod named "cephfs-<%= project.name %>" becomes ready
  ...
  Scenario: CephFS dynamic provisioner
    ...
    When I create a dynamic pvc from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/cephfs/dynamic-provisioning/cephfs_claim.yaml" replacing paths:
    ...
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/cephfs/dynamic-provisioning/pod.json" replacing paths:
      | ["metadata"]["name"]                                         | cephfs-<%= project.name %> |
