Feature: Storage of Ceph plugin testing
  ...
  Scenario: Supporting features parameter in rbd StorageClass
    ...
    When admin creates a StorageClass from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/dynamic-provisioning/storageclass_with_features.yaml" where:
  ...
  Scenario Outline: Supporting fstype parameter in rbd StorageClass
    ...
    Given I have a project
    When admin creates a StorageClass from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/dynamic-provisioning/storageclass_with_fstype.yaml" where:
      | ["metadata"]["name"]          | sc-<%= project.name %>                              |
      | ["parameters"]["monitors"]    | <%= storage_class("cephrbdprovisioner").monitors %> |
    ...
    Given I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/dynamic-provisioning/user_secret.yaml" replacing paths:
  ...
  Scenario: Configure 'Retain' reclaim policy for CephRBD
    ...
    When admin creates a StorageClass from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/dynamic-provisioning/storageclass_retain.yaml" where:
      | ["metadata"]["name"]          | sc-<%= project.name %>                              |
      | ["parameters"]["monitors"]    | <%= storage_class("cephrbdprovisioner").monitors %> |
      | ["parameters"]["imageFormat"] | 1                                                   |
  ...
  Scenario: Using mountOptions for Ceph RBD StorageClass
    ...
    Given I have a project
    When admin creates a StorageClass from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/dynamic-provisioning/storageclass_mount_optins.yaml" where:
      | ["metadata"]["name"]          | sc-<%= project.name %>                              |
      | ["parameters"]["monitors"]    | <%= storage_class("cephrbdprovisioner").monitors %> |
      | ["parameters"]["imageFormat"] | 1                                                   |
  ...
  Scenario: RBD block volumeMode support
    ...
    When admin creates a PV from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/block/pv.json" where:
    ...
    Then the step should succeed
    When I create a manual pvc from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/block/pvc.json" replacing paths:
      | ["metadata"]["name"]       | pvc-<%= project.name %> |
      | ["spec"]["accessModes"][0] | ReadWriteOnce           |
      | ["spec"]["volumeMode"]     | Block                   |
    ...
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/rbd/block/pod.json" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %> |
      | ["spec"]["volumes"][0]["persistentVolumeClaim"]["claimName"] | pvc-<%= project.name %> |
