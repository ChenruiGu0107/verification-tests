Feature: volumeMounts should be able to use subPath
  ...
  Scenario: Subpath should receive right permissions - emptyDir
    ...
    When I run the :create admin command with:
        | f | <%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/emptydir/subpath.yml |
        | n | <%= project.name %>                                                                                        |
  ...
  Scenario: Subpath with downwardAPI volume
    Given I have a project
    When I run the :create client command with:
        | f | <%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/downwardApi-subpath.yaml |
        | n | <%= project.name %>                                                                                                    |
    Then the step should succeed
  ...
  Scenario: Subpath with projected volume
    ...
    When I run the :create client command with:
        | f | <%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/projected-subpath.yaml |
        | n | <%= project.name %>                                                                                                  |
    Then the step should succeed
  ...
  Scenario: Subpath with NFS volume
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/nfs-subpath.json" replacing paths:
  ...
  Scenario: Subpath with iSCSI volume
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/iscsi-subpath.json" replacing paths:
      | ["metadata"]["name"]                                         | mypod |
  ...
  Scenario: Subpath with glusterfs volume
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/glusterfs-subpath.json" replacing paths:
  ...
  Scenario: Subpath with rbd volume
    ...
    Given I switch to cluster admin pseudo user
    And I use the "<%= project.name %>" project
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/rbd-subpath.json" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %> |
      | ["spec"]["volumes"][0]["persistentVolumeClaim"]["claimName"] | pvc-<%= project.name %> |
    Then the step should succeed
    And the pod named "pod-<%= project.name %>" becomes ready
  ...
  Scenario: Subpath with CephFS volume
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/cephfs-subpath.json" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %>        |
  ...
  Scenario Outline: Subpath with cloud volumes
    ...
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/common-subpath.yaml" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %> |
      | ["spec"]["volumes"][0]["persistentVolumeClaim"]["claimName"] | pvc-<%= project.name %> |
  ...
  Scenario: Subpath with sock file
    ...
    Then the step should succeed
    Given I have a project
    When I run oc create over "<%= ENV['BUSHSLICER_HOME'] %>/features/tierN/testdata/storage/subpath/sock-subpath.json" replacing paths:
      | ["metadata"]["name"]                                      | pod-<%= project.name %> |
