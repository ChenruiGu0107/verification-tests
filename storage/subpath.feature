Feature: volumeMounts should be able to use subPath
  ...
  Scenario: Subpath with glusterfs volume
    ...
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/subpath/glusterfs-subpath.json" replacing paths:
  ...
  Scenario: Subpath with rbd volume
    ...
    Given I switch to cluster admin pseudo user
    And I use the "<%= project.name %>" project
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/subpath/rbd-subpath.json" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %> |
      | ["spec"]["volumes"][0]["persistentVolumeClaim"]["claimName"] | pvc-<%= project.name %> |
    Then the step should succeed
    And the pod named "pod-<%= project.name %>" becomes ready
  ...
  Scenario: Subpath with CephFS volume
    ...
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/subpath/cephfs-subpath.json" replacing paths:
      | ["metadata"]["name"]                                         | pod-<%= project.name %>        |
