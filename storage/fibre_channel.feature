Feature: FibreChannel specific scenarios on dedicated servers
  ...
  Scenario: FibreChannel volume plugin with ROX access mode and Retain policy
    ...
    When admin creates a PV from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/fc/pv-retain-rwx.json" where:
      | ["metadata"]["name"]       | pv-<%= project.name %> |
      | ["spec"]["accessModes"][0] | ReadOnlyMany           |
    Then the step should succeed
    When I create a manual pvc from "<%= BushSlicer::HOME %>/features/tierN/testdata/storage/misc/pvc.json" replacing paths:
      | ["metadata"]["name"]       | mypvc        |
      | ["spec"]["accessModes"][0] | ReadOnlyMany |
    Then the step should succeed
