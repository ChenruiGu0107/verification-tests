Feature: testing multicast scenarios
  ...
  Scenario: pods should be able to subscribe send and receive multicast traffic
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh             |
      | exec_command_arg | -c             |
      | exec_command_arg | omping -c 5 <%= cb.pod1ip %> <%= cb.pod2ip %> <%= cb.pod3ip %> >> /tmp/p3.log |
    Then the step should succeed
  ...
  Scenario: pods should be able to join multiple multicast groups at same time
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh             |
      | exec_command_arg | -c             |
      | exec_command_arg | omping -c 5 <%= cb.pod1ip %> <%= cb.pod2ip %> >> /tmp/p1g1.log |
    Then the step should succeed
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh             |
      | exec_command_arg | -c             |
      | exec_command_arg | omping -c 5 -m 232.43.211.235 -p 4322 <%= cb.pod1ip %> <%= cb.pod2ip %> >> /tmp/p1g2.log |
    Then the step should succeed
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh             |
      | exec_command_arg | -c             |
      | exec_command_arg | omping -c 5 <%= cb.pod1ip %> <%= cb.pod2ip %> >> /tmp/p2g1.log |
    Then the step should succeed
    When I run the :exec background client command with:
      | pod              | <%= cb.pod2 %> |
      | oc_opts_end      |                |
      | exec_command     | sh             |
      | exec_command_arg | -c             |
      | exec_command_arg | omping -c 5 -m 232.43.211.235 -p 4322 <%= cb.pod1ip %> <%= cb.pod2ip %> >> /tmp/p2g2.log |
    Then the step should succeed
  ...
  Scenario: pods should not be able to receive multicast traffic from other pods in different tenant
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh                  |
      | exec_command_arg | -c                  |
      | exec_command_arg | omping -c 5 <%= cb.proj1_podip %> <%= cb.proj2_podip %> |
    Then the step should succeed
    ...
    When I run the :exec background client command with:
      ...
      | exec_command     | sh                  |
      | exec_command_arg | -c                  |
      | exec_command_arg | omping -c 5 <%= cb.proj1_podip %> <%= cb.proj2_podip %> >> /tmp/p2.log |
    Then the step should succeed
