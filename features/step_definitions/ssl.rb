...
Given /^the custom certs are generated with:$/ do |table|
  ...
  create_custom_cert_cmd = "oadm ca create-server-cert --signer-cert=#{opts[:signer_cert]} --signer-key=#{opts[:signer_key]} --signer-serial=#{opts[:signer_serial]} --hostnames=#{opts[:hostnames]} --cert=#{opts[:cert]} --key=#{opts[:key]}"
