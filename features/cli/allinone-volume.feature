Feature: All in one volume
  ...
  Scenario: Collisions when one path is explicit and the other is automatically projected
    ...
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-collisions-pod-1.yaml |
    Then the step should succeed
    ...
    Then the output by order should match:
      ...
      | ^644$   |
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-collisions-pod-2.yaml |
    Then the step should succeed
  ...
  Scenario: using a non-absolute path in volumeMounts
    ...
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-opposite-path-pod.yaml |
    Then the step should succeed
  ...
  Scenario: Project secrets, configmap not explicitly defining keys for pathing within a volume
    ...
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-no-keypath-pod.yaml |
    Then the step should succeed
    ...
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/configmap-with-samekey.yaml |
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-no-keypath-pod.yaml |
    Then the step should succeed
  ...
  Scenario: Project secrets, configmap with a same path of keys within a volume
    ...
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-same-path-pod.yaml |
  ...
  Scenario: using invalid volume name
    ...
    Then the step should succeed
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-negative-pod-1.yaml |
    Then the step should fail
    And the output should match:
      | Invalid value: "all-in-one abc" |
      | Not found: "all-in-one abc"     |
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-negative-pod-2.yaml |
    Then the step should fail
    And the output should match:
  ...
  Scenario: Permission mode work well in whole volume and individual resources
    ...
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/allinone-volume/allinone-permission-mode-pod.yaml |
    Then the step should succeed
