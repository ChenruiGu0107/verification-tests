Feature: Audit logs related scenarios
  ...
  Scenario: from log, we can specify what user from the specific IdP did a certain action in the cluster
    ...
    And evaluation of `image_stream("cli", project("openshift")).tags.first.from.name` is stored in the :cli_image clipboard
    Given I have a project
  ...
  # @case_id OCP-34874
  @admin
  Scenario: from log, verify what user from the specific IdP takes a certain action in the cluster
    ...
    # Verify if the normal user login entry captures in oauth-apiserver audit logs
    When admin executes on the pod:
      | bash | -c | oc adm node-logs --role=master --path=oauth-apiserver/audit.log \| grep "<%= cb.users_uid %>" |
    Then the step should succeed
    And the output should contain:
      | "user":{"name":"<%= user.name %>","uid":"<%= cb.users_uid %>"} |
