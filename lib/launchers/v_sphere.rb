...
module CucuShift
  class VSphere
    ...
    # @return [Array] of [RbVmomi::VIM::VirtualMachine, CucuShift::Host] pairs
    ...
    def wait_guest_ip(vm, vmname: nil, timeout: 300)
      ...
      raise CucuShift::TimeoutError, "VM #{vmname} did not get an IP within " \
        "#{timeout} seconds"
    rescue RbVmomi::Fault => e
      if e.message.start_with? "NotAuthenticated:"
        reconnect
...
if __FILE__ == $0
  ...
  conf[:services].each do |name, service|
    if service[:cloud_type] == 'vsphere' && service.dig(:connect, :password)
      vim = CucuShift::VSphere.new(service_name: name)
      res = true
      test_res[name] = res
      begin
