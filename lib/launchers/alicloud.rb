...
module CucuShift
  class Alicloud
    ...
    # @return [Array] of [Alicloud::Instance, CucuShift::Host] pairs
    ...
    def wait_tasks(timeout, region, *ids)
      ...
      unless success
        raise CucuShift::TimeoutError, "Timeout waiting for tasks: #{ids}"
      end
    end
...
if __FILE__ == $0
  ...
  ali = CucuShift::Alicloud.new(service_name: "alicloud")
  ...
  conf[:services].each do |name, service|
    if service[:cloud_type] == 'alibaba'
      ali = CucuShift::Alicloud.new(service_name: name)
      res = true
      test_res[name] = res
      begin
