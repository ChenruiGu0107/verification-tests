    ...
    module Helper
      ...
      def parse_oc_describe(oc_output)
        ...
        res.each do |line|
          if multi_line
            if line.size == 0
              ...
              result[multi_line_keys[0]] += line + "\n"
            end
          else
            name, val = line.split(':')
            if val.nil?
              # multiline output
              multi_line_keys.push(name)
              result[name] = ""
              multi_line = true
            else
        ...
        result[:pods_status] = {:running => pods_status[1], :waiting => pods_status[2],
          :succeeded=>pods_status[3], :failed => pods_status[4]} if pods_status
        result[:replicas_status] = {:current => replicas_status[1], :desired => replicas_status[2]} if replicas_status
        return result
      end
