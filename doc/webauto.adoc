...
open_console_url:
  url: /console
```
Keep in mind that domain name will be included from environment automatically.

=== Elements
To select an element on web page `element` and following `selector` should be
provided. If there is need to select multiple elements, then `elements` and list
of `selector`s should be provided:
```
...
With this property web rule will not fail when required element was not found.
...
`/lib/rules/web/snippets/` directory.
...
== XYAML vs YAML
XYAML format allows us to use multiple values with the same key,
which will be parsed as an `Array` rather than a `Hash` with YAML:
```
parent_action:
  action: first
  action: second
