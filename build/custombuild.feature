Feature: custombuild.feature
  ...
  Scenario: Custom build with imageStreamImage in buildConfig
    ...
    When I process and create "<%= BushSlicer::HOME %>/features/tierN/testdata/build/tc479017/custombuild-template.json"
    Then the step should succeed
  ...
  Scenario: S2I build failure reason display if use incorrect config in buildconfig
    ...
    When I run the :new_app client command with:
      | app_repo | <%= BushSlicer::HOME %>/features/tierN/testdata/build/ruby22rhel7-template-sti.json |
    Then the step should succeed
