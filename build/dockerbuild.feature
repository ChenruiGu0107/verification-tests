Feature: dockerbuild.feature
  ...
  Scenario: Custom build with dockerImage with specified tag
    ...
    And I process and create:
      | f | <%= BushSlicer::HOME %>/features/tierN/testdata/build/tc479296/application-template-custombuild.json |
    Then the step should succeed
  ...
  Scenario: Implement post-build command for docker build
    Given I have a project
    When I run oc create over "<%= BushSlicer::HOME %>/features/tierN/testdata/build/tc479297/test-template-dockerbuild.json" replacing paths:
