Feature: Hugepages related feature
  ...
  Scenario: Hugepages support - Should got fail when pod request a invalid hugepagesize
    ...
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/infrastructure/hugepage/pod_15728.yaml |
      | n | <%= project.name %> |
  ...
  Scenario: Hugepages support - hugepage should work well as a requested resource for pods
    ...
    And I have a project
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/infrastructure/hugepage/pod_15732.yaml |
      | n | <%= project.name%> |
    Then the step should succeed
  ...
  Scenario: Hugepages support - Should fail when pod requests hugepage exceed allocatable resources
    ...
    And I have a project
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/infrastructure/hugepage/pod_15740.yaml |
      | n | <%= project.name %> |
    Then the step should succeed
    And I wait for the steps to pass:
    """
  ...
  Scenario: Hugepages support - Should fail when pod requests multiple hugepagesize
    ...
    And I have a project
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/infrastructure/hugepage/pod_15748.yaml |
      | n | <%= project.name %> |
    Then the step should fail
    And the output should contain:
