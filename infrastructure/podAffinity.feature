Feature: podAffinity
  ...
  Scenario: pod affinity - invalid operator
    ...
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/podAffinity/pod-pod-affinity-invalid-operator.yaml |
  ...
  Scenario: pod affinity - value may not be specified when operator is Exists or DoesNotExist
    ...
    Given I have a project
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/podAffinity/pod-pod-affinity-exists-value.yaml |
    Then the step should fail
    And the output should match:
    ...
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/podAffinity/pod-pod-affinity-doesnotexist-value.yaml |
    Then the step should fail
    And the output should match:
      | [Ff]orbidden.*may not be specified when `operator` is 'Exists' or 'DoesNotExist' |
    When I run the :get client command with:
  ...
  Scenario: pod affinity topologykey cannot be empty
    ...
    Given I have a project
    When I run the :create client command with:
      | f | https://raw.githubusercontent.com/openshift-qe/v3-testfiles/master/pods/podAffinity/pod-pod-affinity-invalid-topologykey-empty.yaml |
    Then the step should fail
    And the output should match:
  ...
  Scenario Outline: pod prefers to be scheduled to the nodes which matches affinity rules cases for 4.x
    ...
    Examples:
      ...
      | scheduler_interpodaffinitypriority_10.json  | security-s1   | pod-s1.yaml               | pod-affinity-in-s1-preferred      | pod-pod-affinity-in-s1-preferred.yaml       | ==        | # @case_id OCP-26285
      | scheduler_interpodaffinitypriority_10.json  | security-s1   | pod-s1.yaml               | pod-anti-affinity-in-s1-preferred | pod-pod-anti-affinity-in-s1-preferred.yaml  | !=        | # @case_id OCP-25870
